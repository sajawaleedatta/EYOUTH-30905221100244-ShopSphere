...
**2. Background tasks use Vercel Serverless Functions.** A new `api/notification.js` serverless function handles order confirmations, review notifications, and low-stock alerts as fire-and-forget background jobs, invoked via HTTP POST. This decouples notification logic from the request cycle.
...
- **Negative:** Network hop for review requests (mitigated by same-region deployment), shared JWT secret must be synchronized, one more service to monitor.
